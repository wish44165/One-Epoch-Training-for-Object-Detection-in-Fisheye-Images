                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        if save_txt:  # Write to file
                            ...
                            #preds = ('%g ' * len(line)).rstrip() % line
                            #preds = preds.split(' ')
                            #print(preds)     # class, ratio of x, ratio of y, ratio of w, ratio of h
                            #preds[1:] = round(float(preds[1]) * imx), round(float(preds[2]) * imy), round(float(preds[3]) * imx), round(float(preds[4]) * imy)
                            if preds[1] < 1:
                                preds[1] = 1
                            if preds[2] < 1:
                                preds[2] = 1
